...
# - Do not implement MinerU business logic in PowerShell
...
function Show-Usage {
    @"
Usage: run-mineru.ps1 [options] [-- <extra-cli-args>]

Options:
  --input DIR                Input directory (default: inbox)
  --output DIR               Output directory (default: out)
  --model-version VALUE      pipeline | vlm | MinerU-HTML (default: pipeline)
...
function Resolve-ExistingDirPath {
    param([Parameter(Mandatory = $true)][string]$PathValue)

    $resolved = Resolve-DirPath -PathValue $PathValue
    if (-not (Test-Path -LiteralPath $resolved -PathType Container)) {
        Exit-WithError -Message "Error: Input directory does not exist: $PathValue" -Code 1
    }
...
while ($index -lt $argv.Count) {
    ...
    switch ($arg) {
        ...
        "--model-version" {
            if ($index + 1 -ge $argv.Count) {
                Exit-WithError -Message "Error: --model-version requires a value" -Code 2
            }
            $modelVersion = [string]$argv[$index + 1]
            $index += 2
            continue
        }
        "--config" {
...
$inputDir = Resolve-ExistingDirPath -PathValue $inputDir
...
$cmdArgs = @("-m", "mineru_batch_cli", "run", "--input", $inputDir, "--output", $outputDir, "--model-version", $modelVersion, "--continue-on-error", $continueOnError)
